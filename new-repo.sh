...
# the repo CLAUDE.md template, the declaration it extends).
...
PUBLIC_REQUIRED_CONTEXTS='["floor / floor","trusted-scan / trusted-scan"]'
# Why this list exists with no CODEOWNERS file behind it: the ruleset's
# `codeowners_*` map is Margot's owned-tier INPUT (estate-gate.yml derives a
...
for f in "$PROVISIONER" "$PCC_MERGE_PY" "$DECLARED_JSON_PATH" "$CLAUDE_TEMPLATE" "$DOTTY_CHECKOUT/$GATE_EVAL_REL" \
...
One concern: this repository's declaration. \`rulesets/default-branch.json\` gains \`.repos["$REPO_SLUG"]\` — the standard profile (\`floor / floor\` + \`trusted-scan / trusted-scan\` — the two-lane floor; \`margot_enrolled\`; the gate machinery as \`codeowners_owned\`, Margot's owned-tier input — no CODEOWNERS file is rendered from it)$private_note. $eval_line
...
Generated mechanically by \`new-repo.sh\`, covered by \`.claude/eval/new-repo.test.sh\` (declaration shape for a public and a private repository, key order, the eval list edit). This PR's own required checks apply: \`floor / floor\`, \`trusted-scan / trusted-scan\`, \`eval-suite\` (which runs the edited gate eval against this declaration), and Margot's review.
