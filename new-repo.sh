...
if [[ "$VISIBILITY" == private ]]; then
	NEW_ENTRY="$(jq -n --argjson rc "$PUBLIC_REQUIRED_CONTEXTS" --argjson co "$SEED_CODEOWNERS_OWNED" \
...
One concern: this repository's declaration. \`rulesets/default-branch.json\` gains \`.repos["$REPO_SLUG"]\` — the standard profile (\`all-checks-passed\`, \`trusted-scan / trusted-scan\`; \`margot_enrolled\`; the seed's CODEOWNERS owned set)$private_note. $eval_line

Generated by \`new-repo.sh\`, which also seeded the repository (thin \`@v1\` callers, the standard pre-commit suite, \`.gitleaks.toml\`, \`.house-code.json\`, CODEOWNERS, README, CLAUDE.md$license_note) and opened the companion callers PR in the repository itself.
