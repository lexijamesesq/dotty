...
process_callers() {
	...
	local url
	if [[ -n "$existing" ]]; then
		# The description must describe THIS plan, not the one that opened the
		# PR: refresh title and body so a re-run never leaves a stale narrative
		# for Margot to hold (core-skills #114).
		"$GH" api -X PATCH "repos/$REPO_SLUG/pulls/$existing_num" \
			-f "title=$(caller_pr_title)" -f "body=$(caller_pr_body)" >/dev/null 2>&1 ||
			echo "  WARN  $REPO_SLUG: could not refresh the open PR's title/body" >&2
		printf '  PR    updated %s\n' "$existing"
...
# caller_pr_body — the seven-heading estate body (pr-body:v1), built from THIS
# run's plan. The first rollout's body narrated the first rollout (the
# dependency pipe); every later converge inherited that narrative, so a pull
# request that only added one caller described a migration it did not make.
# Margot held it as misrepresented (core-skills #114). "What changed" is now
# the PLAN lines and the deletions, nothing else; the standing paragraphs say
# only what is true of every converge. The author is not a declared
# dependency bot, so the CI body check applies and Margot reviews this like
# any other workflow change. Carries no secret and reads none.
caller_pr_title() {
	printf 'callers: converge this repository to the estate-owned surfaces'
}
caller_pr_body() {
	local i reasons="" deletions=""
	for i in "${!CALLER_REASONS[@]}"; do reasons+="- ${CALLER_REASONS[$i]}"$'\n'; done
	if [[ ${#CALLER_DELETES[@]} -gt 0 ]]; then
		for i in "${!CALLER_DELETES[@]}"; do deletions+="- \`${CALLER_DELETES[$i]}\` is **deleted** (the estate retired it; nothing reads it)."$'\n'; done
	else
		deletions="- None."$'\n'
	fi
	cat <<BODY_EOF
<!-- pr-body:v1 -->
## Intent
Bring this repository's estate-owned surfaces to the shape dotty declares, so it stays on the estate's CI, review and merge pipe without a hand edit here. Generated by \`provision-public-repo.sh --callers\`, which owns these surfaces so a repo drifting off the shape is reported as drift by the daily check instead of failing silently at the next release.

## What changed
Exactly the plan this run computed against this repository's current bytes:
...
## Rollback
Revert this commit and merge the revert; the surfaces return to their previous bytes and any deleted file comes back. The next converge will propose the same change again until dotty's declaration changes.

## Ticket
None — the estate provisioner's caller converge, run by the operator's session after a dotty release.
