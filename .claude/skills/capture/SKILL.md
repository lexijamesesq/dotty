...
- **The calibration surface supplies the judgment** — kind definitions, dimensions, thresholds: `{workspace_root}/Wiki/claude/skills/knowledge-integration/calibration-surface.md` (resolve the vault root via the `workspace_root` config key; lives in the [wiki](https://github.com/lexijamesesq/wiki) repo — public companion, not shipped in this repo). Kind is a PROPOSAL — the gatekeeper may re-grade.
...
- `{workspace_root}/Wiki/claude/skills/knowledge-integration/SKILL.md` — the gatekeeper (candidate schema, disposition machinery). Lives in the [wiki](https://github.com/lexijamesesq/wiki) repo — public companion, not shipped in this repo.
- `{workspace_root}/Wiki/claude/skills/knowledge-integration/calibration-surface.md` — canonical judgment tables. Same companion repo.
