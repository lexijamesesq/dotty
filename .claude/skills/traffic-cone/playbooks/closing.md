...
- **REFUTED cap: 3 cycles per operator direction — counted from the ticket's `[VALIDATION]` REFUTED comments since her latest direction on the ticket, never from session memory** (a park and re-claim does not reset the count — the count lives on the ticket, read via `@linear`, not in this orchestration's own memory). At the cap, delegate to `@linear`: move the ticket to **Needs Input** with a comment summarizing the impasse (releases the claim, posts resume state). The cap flags runaway rounds; it never ends the road — her direction returns the ticket to the frontier and restarts the count.
