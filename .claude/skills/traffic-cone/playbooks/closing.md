...
**Step 2 — Gate.** The `` `@attack-kitty` `` spawn from Step 1 returns its verdict directly — CONFIRMED returns the verdict word plus the id of the `[VALIDATION]` comment it posted; any other verdict returns the full verdict block (Checked, Verdict, Specifics, Intent, Not covered, Mode) with nothing posted to Linear. Read the verdict from that return value, not from a Linear comment.

- `CONFIRMED` → proceed to Step 3.
- `CONFIRMED-WITH-GAPS` → resolve each named gap before transition, using the Specifics from the returned block. A gap is orphaned, not owned — the receiving session fixes it in-session. After all gaps are resolved, re-invoke `mark_done` (same re-check path as REFUTED).
- `REFUTED` → return the specifics from the returned block to the caller. Ticket stays In Progress. Named substitute: apply the validator's specifics, then re-invoke `mark_done`. On re-invocation, spawn a fresh `` `@attack-kitty` `` with the same mandate inputs — never resume the prior validator. Carry the prior verdict's specifics forward in this orchestration's own working context to scope the fresh spawn's re-check to the named findings — there is no ticket comment to read them back from. The latest verdict governs; prior verdicts are scoping input only.
...
- **REFUTED cap: 3 cycles per continuous `mark_done` orchestration, counted in this session's own working context — never by reading Linear** (a REFUTED verdict is never posted as a comment, so there is nothing on the ticket to count). Each REFUTED-driven re-invocation within this same orchestration run increments the count. A park to Needs Input ends the run — a fresh claim and a fresh `mark_done` invocation start a fresh count; the cap flags runaway rounds within one continuous sitting, not drift across sessions. At the cap, delegate to `@linear`: move the ticket to **Needs Input** with a comment summarizing the impasse (releases the claim, posts resume state).
