...
2. **Traffic-cone's gate surfaces are the gate.** Before any mutation of a foundation record, `cone_preflight.py`'s mark_done checks (the M-series, incl. the M2.5 charter check) and `close-map.md` check for the marker and, if present, verify one of the two authorization receipts exists before executing the mutation. This is the same admission-test posture the M2.5 charter check already carries (verbatim, pinned, FINALIZED-gated) — this spec generalizes that existing discipline to every foundation-class record, present and future, rather than leaving it charter-specific tribal knowledge.
