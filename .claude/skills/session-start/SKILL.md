...
**Strategic context.** Harness-side implementation of the session-bootstrap pattern from `[[sustained-autonomous-agentic-workflows]]`. Read interface to the three-layer memory architecture. Sits at the head of every working session across the operator's portfolio (System + Strategy + Team + Metrics + Incubator + Router + HA + Recruiting + Threat Assessment).
