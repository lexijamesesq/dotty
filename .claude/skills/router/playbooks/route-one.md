...
- **Project-owned:** Task axis → Linear per `{workspace_root}/System/intake-defaults.md` + `linear-discipline` (context doc for 3+ sentence captures). Knowledge axis → only with a declared `### Knowledge` Intake subsection, per spec § Knowledge Delivery Rules (envelope, `sources` provenance, index update, filing-validator; missing location → queue, never create).
