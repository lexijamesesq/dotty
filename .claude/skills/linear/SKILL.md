...
Takeable = Todo, unblocked, unassigned, unclaimed, not labeled `map`, and **not a child of a map** — map children belong to map sessions, reached via `read map-frontier` and routed by type label, never by the generic flow — with one exception: a `build` child labeled `ready-for-agent` is takeable, and its claimant becomes the ticket's conductor (claim's build variant routes to `/conduct`). The claim is stored in the `delegate` field; the `assignee` field is system-set on operator-directed claims (a record of co-engagement) and is the operator's field to clear. Ordering: priority (Urgent → Low), then age (oldest first). `read map-frontier` and `read project-frontier` serve this convention as reads; `` `@traffic-cone` ``'s `work frontier` orchestration is what picks, claims, and drives one ticket to Done per session.
