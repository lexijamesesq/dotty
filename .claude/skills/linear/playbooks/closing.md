...
- `CONFIRMED-WITH-GAPS` → resolve each named gap before transition. A gap sitting in the validator's comment is orphaned, not owned. Fix in-session — the session receiving the verdict is accountable. After all gaps are resolved, re-invoke `mark_done`; continue the same validator via SendMessage for a scoped re-check (same path as REFUTED re-invocation).
