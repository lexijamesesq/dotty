...
The closure verbs — `mark_done` (gates the Done transition on non-author validation) and `resolve` (the close for decision-type map children). Split from `issue-management.md` at the pre-identified seam; callers invoke them through the same batched-mutation interface, and the input fields (`validation_type`, `evidence`, `charter_doc_id`, `body`) ride that playbook's mutation schema. Team/stateId resolution follows the SKILL.md cross-cutting rules, cached per invocation.
