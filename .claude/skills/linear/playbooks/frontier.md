...
**`delegate` isn't exposed by the tactic MCP** — every takeable-set query below goes through the GraphQL bridge (`~/.config/op-agent/bin/linear-gql`, same bridge as `playbooks/claim.md`'s Step 6), never `linear_getIssues`/`linear_getProjectIssues` alone.
...
- Does NOT pick or claim a ticket — that's `` `@traffic-cone` ``'s `work frontier` orchestration (picks, calls `playbooks/claim.md`, drives to close, one ticket per session).
