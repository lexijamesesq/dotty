...
Identify `Todo` and `In Progress` items past per-priority freshness thresholds based on `updatedAt`. Stale here means "should be moving but isn't" — distinct from `Needs Input`/`Blocked` which aren't expected to move.
...
### 2. Needs Input/Blocked re-evaluation
