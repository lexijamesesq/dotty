...
- **State on pick-up reciprocal**: `claim` opens the loop — the claim lives in the delegate field (the assignee field is the operator's hold), with variant discipline per ticket shape; `mark_done` closes it through the non-author validation gate; decision-type map children close through `resolve` — the researcher's own completion for research, the map session's live exchange for HITL types, never closeout's verb; work that won't be done through `cancel`. All per `/linear`'s Navigation.
