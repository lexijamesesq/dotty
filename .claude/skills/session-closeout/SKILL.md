...
4. `/linear update issues` with item-level mutations: `mark_done`, `comment`, `move_state`, `create_followup`, `update_description` as appropriate.
...
- **State on pick-up reciprocal**: `mark_done` at closeout closes the loop.
