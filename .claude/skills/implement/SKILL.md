...
4. **Manual proof items name their release evidence.** A manual item is released by a form the estate produces, never by a comment authored by the operator's own Linear user. If she authorized this work in-session, record that as a claimed-and-dated note that cites its corroborating anchor — the session/exchange, a witnessed-run/wake-record id, or a specific gate-trail comment id — and that dated, anchored authorization is the receipt; the item needs no park. If it awaits something only she can produce — a run she must witness — run traffic-cone's fused `park` script with a what-to-look-at note (via `--comment-file`) that names the producible release form: the witnessed-run artifact to point back to, then "tell any session to resume <ticket>." Never assert a manual item satisfied yourself; the validator probes for the anchored authorization, the witnessed artifact, or its own outcome validation — not a comment in your voice.
