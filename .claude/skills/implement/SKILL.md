...
5. **Validate.** Spawn `` `@attack-kitty` `` with the validation mandate the proof names and the charter's pinned document id. Attack-kitty fetches its own evidence, re-runs the proof, and judges the work against the charter and the ticket's Done When. Three outcomes:
...
Every park goes through `` `@traffic-cone` `` — the conductor names the reason and the ask; traffic-cone verifies the ask is specific, executes the state change, and releases the claim. To resume: the operator confirms the blocker is resolved (or directs the un-park), a session spawns `` `@traffic-cone` `` `un-park` (Needs Input → Todo), then re-claims through this skill's pre-flight and claim step and continues from the ticket's parking note. Park when: manual proof awaits the operator; the retry is spent; a charter claim fails against a changed fact — attach the receipt, and post one `[CHALLENGE]`-prefixed comment on the map: claim refuses new build dispatches while one stands open, so the challenge actually stops the lane. A challenge is open until an operator-directed `[CHALLENGE-RESOLVED]` reply carries the adjudication gist — that reply is the release; without it the lane stays halted.
