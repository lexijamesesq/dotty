...
- integration-mode guard (automated, before any append): read the destination's `integration:` frontmatter override ([[integration-modes]] §3; class default §2). `integration: current-truth` target → mutation surface; the validated-mutation path ([[integration-modes]] §4–§5) is INACTIVE → disposition `queue`, reason `mutation-path-inactive` (never an automated append to a current-truth surface). Evolution target → proceed to append.
