...
6. **After Done: check the siblings.** If no open sibling ticket of any type remains on the map (the map lane's close requires zero open children), post one comment on the map — "last build ticket closed; ending due" — the signal for a map session to run `/linear close-map`.
