...
5. **Add and graduate.** Add newly-surfaced tickets (create-then-wire, per SKILL.md § Cutting discipline). Graduate any fog the answer made specifiable, clearing each graduated patch from **Not yet specified** so it lives only as its new ticket. A ticket sitting beyond the destination → rule it out of scope (SKILL.md § Out of scope), never resolve it on the route. If the decision invalidates other parts of the map, update those tickets or propose cancellation to the operator; on confirmation, run the `cancel` transition (with reason).
