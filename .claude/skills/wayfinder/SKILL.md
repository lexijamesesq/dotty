...
Every lifecycle transition goes through `` `@traffic-cone` ``; every non-author validation goes through `` `@attack-kitty` ``. Both carry their invocation protocol in their `## Invocation` section and can answer "how do I work with you" at runtime. If you're unsure how to compose a spawn prompt, **ask the agent**.
