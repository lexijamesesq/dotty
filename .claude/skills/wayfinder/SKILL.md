...
- **Researcher** — the spawned agent resolving one `research` ticket, blind to the map. Spawns at `sonnet` absent a `model:*` label.
...
Two modes. Either way, **never resolve more than one ticket per session** — with the exception of `research`+`afk` tickets: the map session runs those as the **orchestrator, never the researcher** (see work-through step 3), and may chain up to three as they unblock. A `research`+`hitl` ticket is a HITL resolution like any other: one per session.
