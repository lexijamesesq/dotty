...
description: >
  ...
  file against the knowledge-contract Part II envelope plus the relevant filing-handoff §,
...
- **The envelope, not the content.** You check mechanically-verifiable static
  ...
  should mutate, whether it will go stale, topic quality, or writing. `knowledge-contract Part II`
...
2. **Handoff §** — which filing handoff produced it: `§1 wiki intake`,
   `§2 project knowledge filing`, `§4 session capture (incl. closeout
   query-and-file — the former §3, folded in)`, or `§5 automated capture`.
...
2. **`Wiki/spec/knowledge-contract.md § Part II`** — the envelope rules. Its **Parsing Contract**
...
3. **`Wiki/spec/knowledge-contract.md § Part III`** — read the one § the caller named (§1, §2, or §3).
   This adds the handoff-specific field-derivation expectations on top of the envelope.

Tag-validity rules are delegated by `knowledge-contract Part II` to `knowledge-contract Part I`; if you
need to resolve whether a specific tag is valid, read `Wiki/spec/knowledge-contract.md § Part I` and
apply its own parsing contract.

## Check procedure

Derive every check from `knowledge-contract Part II`'s Parsing Contract — never from a
...
- All tags valid per `knowledge-contract Part I` (namespaces, closed vocabularies, depth limits).
...
- If the type appears in `knowledge-contract Part II`'s **Scope Boundaries** list, it is
...
Do NOT check for an `index.md` entry. `knowledge-contract Part IV` scopes the
...
From the named § in `knowledge-contract Part III`, confirm the handoff-specific delta values are
...
Use the severities `knowledge-contract Part IV` assigns each check:
...
- **WARNING / INFO** — anything `knowledge-contract Part IV` marks below HIGH for a filing-time check.

The filing-time surface (`knowledge-contract Part IV` rows with Mode `Filing` or `Both`) carries no
MEDIUM checks — every envelope check is HIGH. MEDIUM-severity rows in `knowledge-contract Part IV`
...
`knowledge-contract Part IV` escalates them; at filing time on a brand-new file there is no legacy
...
- Each violation names the **rule** (so the caller knows what to fix) and the **source
  contract** (SC = knowledge-contract Part II, HC = knowledge-contract Part III, TT = knowledge-contract Part I).
