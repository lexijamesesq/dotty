...
[[ -f "$TMPSENT" ]] && pass "sentinel file preserved after read (LEX-144 — no auto-delete)" || fail "sentinel file should be preserved, not deleted"
rm -f "$TMPSENT"

section "sentinel preserved on consecutive reads (LEX-144 chained-call fix)"
TMPSENT="$(mktemp -t resolvepath.XXXXXX)"
echo "/tmp" > "$TMPSENT"
expected="$(realpath /tmp)"
...
section "JSON-format sentinel with fresh created_at (LEX-144)"
TMPSENT="$(mktemp -t resolvepath.XXXXXX)"
...
section "JSON-format sentinel with stale created_at (>5 min) falls through (LEX-144)"
TMPSENT="$(mktemp -t resolvepath.XXXXXX)"
