#!/usr/bin/env bash
# Test suite for the "Post the queued `margot` check" step of
# .github/workflows/estate-margot.yml — the dispatch's promise that a pull
...
WORKFLOW="${WORKFLOW:-${SCRIPT_DIR}/../../.github/workflows/estate-margot.yml}"
[[ -f "$WORKFLOW" ]] || {
...
for job in wf["jobs"].values():
    for step in job.get("steps", []):
        if step.get("name", "") == "Post the queued `margot` check":
...
section "dispatch posts ONE queued $(margot) check-run on the target head"
run_step
...
assert_eq "status is queued (not in_progress, not completed)" "queued" "$(jq -r .status "$TMP/body.json")"
[[ "$(jq -r '.conclusion // "none"' "$TMP/body.json")" == "none" ]] && pass "no conclusion on a queued check" || fail "no conclusion on a queued check" "$(cat "$TMP/body.json")"
...
grep -q 'posted queued margot check for acme/widgets PR #7' <<<"$OUT" && pass "logs what it posted" || fail "logs what it posted" "$OUT"
