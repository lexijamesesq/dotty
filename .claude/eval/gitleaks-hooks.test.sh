...
git_init_repo "$NOUP"
write_config_chain "$NOUP"
echo "solo" >"$NOUP/s.txt"
git -C "$NOUP" add s.txt .gitleaks.toml
git -C "$NOUP" commit -q -m base --no-verify
(cd "$NOUP" && env XDG_CONFIG_HOME="$XDG_CONFIG_HOME" bash "$PREPUSH") >/dev/null 2>"$ERRFILE"
RC=$?
...
NB="$TMP/newbranch"
NB_ORIGIN="$TMP/newbranch-origin.git"
git_init_repo "$NB"
write_config_chain "$NB"
echo "clean base" >"$NB/a.txt"
git -C "$NB" add a.txt .gitleaks.toml
git -C "$NB" commit -q -m base --no-verify
git clone -q --bare "$NB" "$NB_ORIGIN"
git -C "$NB" remote add origin "$NB_ORIGIN"
git -C "$NB" fetch -q origin
git -C "$NB" checkout -q -b feature
printf 'leak %s\n' "$CANARY" >"$NB/f.txt"
git -C "$NB" add f.txt
git -C "$NB" commit -q -m feat --no-verify
(cd "$NB" && env XDG_CONFIG_HOME="$XDG_CONFIG_HOME" bash "$PREPUSH") >/dev/null 2>"$ERRFILE"
RC=$?
...
git_init_repo "$FTP"
write_config_chain "$FTP"
echo "clean base" >"$FTP/a.txt"
git -C "$FTP" add a.txt .gitleaks.toml
git -C "$FTP" commit -q -m base --no-verify
git clone -q --bare "$FTP" "$FTP_ORIGIN"
git -C "$FTP" remote add origin "$FTP_ORIGIN"
git -C "$FTP" fetch -q origin
mkdir -p "$FTP/git-hooks"
cp "$HOOKS_DIR"/gitleaks-*.sh "$FTP/git-hooks/"
chmod +x "$FTP/git-hooks/"*.sh
...
printf 'key = %s\n' "$CANARY" >"$FTP/leak.txt"
git -C "$FTP" add leak.txt
git -C "$FTP" commit -q -m oops --no-verify
...
echo "harmless" >"$FTP/ok.txt"
git -C "$FTP" add ok.txt
git -C "$FTP" commit -q -m ok --no-verify
...
MSG_BAD="$TMP/msg-bad.txt"
MSG_OK="$TMP/msg-ok.txt"
printf 'Add feature\n\nleftover key %s\n' "$CANARY" >"$MSG_BAD"
...
(cd "$REPO" && env XDG_CONFIG_HOME="$XDG_CONFIG_HOME" GL_NO_OVERLAY=1 GL_CONFIG_PATH="$PINNED_TOKEN_CONFIG" \
	GL_TEXT_FILE="$TMP/pinned-probe.txt" bash "$COMMITMSG") >/dev/null 2>"$ERRFILE"
RC=$?
...
git -C "$REPO" checkout -q main
echo "token FIXEDPATHMARKER staged" >"$REPO/staged-bad.txt"
git -C "$REPO" add staged-bad.txt
run_staged
...
git -C "$REPO" reset -q staged-bad.txt
rm -f "$REPO/staged-bad.txt"
...
echo "nothing to see" >"$REPO/staged-ok.txt"
git -C "$REPO" add staged-ok.txt
run_staged
...
git -C "$REPO" reset -q staged-ok.txt
rm -f "$REPO/staged-ok.txt"
...
echo "token FIXEDPATHMARKER ignorable" >"$REPO/ignorable.txt"
git -C "$REPO" add ignorable.txt
...
git -C "$REPO" reset -q ignorable.txt
rm -f "$REPO/ignorable.txt"
...
rm -f "$PRIVREPO/other.txt" "$PRIVREPO/cred.txt"
git -C "$PRIVREPO" add -A
...
printf 'key %s\n' "$CANARY" >"$PRIVREPO/cred.txt"
git -C "$PRIVREPO" add -A
(cd "$PRIVREPO" && env XDG_CONFIG_HOME="$XDG_CONFIG_HOME" GL_NO_OVERLAY=1 bash "$STAGED") >"$ERRFILE" 2>&1
RC=$?
...
git_init_repo "$CLAIMREPO"
write_config_chain "$CLAIMREPO"
...
(cd "$CLAIMREPO" && env XDG_CONFIG_HOME="$XDG_CONFIG_HOME" bash "$STAGED") >"$ERRFILE" 2>&1
RC=$?
