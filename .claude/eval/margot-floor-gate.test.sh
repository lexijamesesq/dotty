...
[[ -f "$S" ]] || {
	echo "FATAL: missing $S"
	exit 2
}
[[ -f "$RS" ]] || {
	echo "FATAL: missing $RS"
	exit 2
}
...
[{"name":"all-checks-passed","status":"in_progress","conclusion":null}]
EOF
...
# dotty-private requires all-checks-passed + eval-suite (the floor-first contexts flip per repo in the caller batch); with both green (and margot
# absent), it admits.
...
# `all-checks-passed` — cancelled, started 21:35:42, and success,
