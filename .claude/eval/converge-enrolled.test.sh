...
declare_repos() {
	{
		...
		printf '}}'
	} >"$DOTTY/rulesets/default-branch.json"
}
run_converge() { # <mode>
	OUT="$(PROVISION_CMD="$PROV" bash "$SCRIPT" "$1" "$DOTTY" 2>&1)"
	RC=$?
}
...
new_case
declare_repos "lexijamesesq/alpha"
printf '  DRIFT rule.pull_request = review_count=2 (intended 1)\n' >"$TMP/out.lexijamesesq_alpha"
echo 1 >"$TMP/rc.lexijamesesq_alpha"
run_converge check
...
new_case
declare_repos "lexijamesesq/alpha"
OUT="$(PROVISION_CMD="$PROV" bash "$SCRIPT" sideways "$DOTTY" 2>&1)"
RC=$?
assert_eq "an unknown mode refuses (exit 2), never guesses" "2" "$RC"
if [[ -s "$CALLS" ]]; then
	fail "an unknown mode touches nothing" "$(cat "$CALLS")"
else pass "an unknown mode touches nothing"; fi

new_case
rm -f "$DOTTY/rulesets/default-branch.json"
OUT="$(PROVISION_CMD="$PROV" bash "$SCRIPT" converge "$DOTTY" 2>&1)"
RC=$?
assert_eq "a missing declaration refuses (exit 2)" "2" "$RC"
if [[ -s "$CALLS" ]]; then
	fail "a missing declaration touches nothing" "$(cat "$CALLS")"
